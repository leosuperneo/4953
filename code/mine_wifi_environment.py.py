...
class MineEnvironment:
    ...
    def __init__(self):
        ...
        # 状态参数
        self.current_ap_idx = 0  # 当前连接的AP索引
        self.target_ap_idx = self.num_aps - 1  # 目标AP（最右侧主矿道末端）
        self.position = np.array(self.aps[0][:2], dtype=float)  # 当前STA位置
        self.target_position = None
        self._find_target_ap()  # 找到最右侧的AP作为目标
    ...
    def reset(self) -> np.ndarray:
        """重置环境"""
        self.current_ap_idx = 0
        self.position = np.array(self.aps[0][:2], dtype=float)
        self.steps = 0
        self.handover_count = 0
        self.total_signal_quality = 0
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        if distance_to_target > 0:
            ...
            move_direction = direction / distance_to_target
...
def train_dqn(num_episodes: int = 1000):
    """训练DQN"""
    env = MineEnvironment()
...
def visualize_path(env, agent, num_steps=100):
    ...
    for i, ap in enumerate(env.aps):
        color = 'red' if i == env.target_ap_idx else 'blue'
        size = 200 if i == env.target_ap_idx else 100
        ax.scatter(ap[0], ap[1], c=color, s=size, alpha=0.6, marker='s', edgecolors='black')
...
if __name__ == "__main__":
    ...
    # 训练
    print("\n开始训练...")
    agent, env = train_dqn(num_episodes=500)
