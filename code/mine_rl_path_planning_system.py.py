...
class MineEnvironment:
    ...
    重要特性：
    - 考虑矿道物理结构的通信模型
    - 不同矿道间有岩壁阻挡，信号会有巨大衰减（40dB穿墙损耗）
    - STA需要沿着矿道结构移动，而不能直接穿墙
    - 这确保了路径规划的真实性和合理性
    """
    ...
    def __init__(self, start_ap_idx=0, target_ap_idx=None, branch_config=None):
        ...
        print(f"\n通信模型特性:")
        print(f"  ✓ 同一矿道内: 正常路径损耗 (路径损耗指数=3.5)")
        print(f"  ✓ 穿过岩壁: 额外40dB衰减 + 更大路径损耗 (指数=5.5)")
        print(f"  ✓ 这确保了路径必须沿着矿道结构，而不能直接穿墙")
        print(f"{'='*60}\n")
    ...
    def _are_in_same_tunnel(self, ap1_pos: np.ndarray, ap2_pos: np.ndarray) -> bool:
        """判断两个位置是否在同一条矿道内（无岩壁阻挡）"""
        x1, y1 = ap1_pos
        x2, y2 = ap2_pos
        
        # 情况1：两个都在主矿道上（y=0）
        if abs(y1) < 1 and abs(y2) < 1:
            return True
        
        # 情况2：两个都在同一个分支矿道上（x坐标相同，y符号相同）
        if abs(x1 - x2) < 1:  # 同一个x位置（分支点）
            # 检查是否在同一侧（都是正y或都是负y）
            if (y1 > 0 and y2 > 0) or (y1 < 0 and y2 < 0):
                return True
        
        # 情况3：一个在主矿道，一个在分支矿道的入口处
        # 如果分支AP距离主矿道很近（刚进入分支），可以认为信号可达
        if abs(y1) < 1 or abs(y2) < 1:  # 一个在主矿道
            # 检查分支AP是否在分支入口附近（y距离小于100米）
            if abs(y1) < 100 and abs(y2) < 100:
                # 并且x坐标匹配某个分支点
                for branch in self.branch_config:
                    if abs(x1 - branch['x']) < 1 or abs(x2 - branch['x']) < 1:
                        return True
        
        # 其他情况：不在同一矿道内，有岩壁阻挡
        return False
    
    def _calculate_path_loss(self, distance: float, through_wall: bool = False) -> float:
        """计算路径损耗 (Log-distance path loss model)
        
        参数:
            distance: 距离（米）
            through_wall: 是否穿过岩壁（不在同一矿道）
        """
        if distance < 1:
            distance = 1
        ...
        if through_wall:
            # 穿过岩壁：巨大的额外衰减
            n = 5.5  # 更大的路径损耗指数
            X_sigma = np.random.normal(0, 8)  # 更大的阴影衰落
            wall_loss = 40  # 岩壁穿透损耗：40dB（非常大）
        else:
            # 同一矿道内：正常衰减
            n = 3.5  # 路径损耗指数
        ...
        path_loss = pl_d0 + 10 * n * np.log10(distance / d0) + X_sigma + wall_loss
        
        return path_loss
    
    def _calculate_rssi(self, ap_idx: int) -> float:
        """计算RSSI (接收信号强度指示)，考虑矿道结构"""
        ap_pos = np.array(self.aps[ap_idx][:2])
        distance = np.linalg.norm(self.position - ap_pos)
        
        if distance > self.max_range:
            return -100  # 超出范围
        
        # 判断是否在同一矿道内
        in_same_tunnel = self._are_in_same_tunnel(self.position, ap_pos)
        
        # 计算路径损耗（如果不在同一矿道，会有巨大的穿墙损耗）
        path_loss = self._calculate_path_loss(distance, through_wall=not in_same_tunnel)
    ...
    def step(self, action: int) -> Tuple[np.ndarray, float, bool, Dict]:
        ...
        # 记录移动前到目标的距离（用于计算进度奖励）
        distance_to_target = np.linalg.norm(self.target_position - self.position)
        
        # STA向当前连接的AP移动（沿着信号最强的方向）
        # 而不是直接向目标移动，避免穿墙
        current_ap_pos = np.array(self.aps[self.current_ap_idx][:2])
        
        # 如果离当前AP太远，先靠近当前AP
        distance_to_current_ap = np.linalg.norm(self.position - current_ap_pos)
        
        if distance_to_current_ap > 60:  # 距离当前AP太远
            # 向当前AP移动
            direction = current_ap_pos - self.position
        else:
            # 向目标方向移动，但沿着矿道结构
            ...
            # 如果在主矿道（y接近0），主要沿x方向移动
            if abs(y) < 30:
                direction = np.array([target_x - x, np.sign(target_y - y) * min(abs(target_y - y), 10)])
            # 如果在分支矿道，检查是否需要先回到主矿道
            else:
                # 找到最近的主矿道位置（回到y=0）
                if abs(y) > 50 and abs(target_y) < 30:  # 目标在主矿道，自己在分支
                    # 先向主矿道移动
                    direction = np.array([0, -np.sign(y) * 15])
                else:
                    # 沿着当前分支移动
                    direction = np.array([np.sign(target_x - x) * min(abs(target_x - x), 10), 
                                         np.sign(target_y - y) * 15])
        
        distance = np.linalg.norm(direction)
        
        if distance > 0:
            step_size = 10  # 每步移动10米
            move_direction = direction / distance
            
            # 移动，但不超过目标
            if distance > step_size:
                self.position += move_direction * step_size
            else:
        ...
        if current_sinr > 20:
            ...
            reward -= 10  # 严重惩罚信号很差的情况（可能穿墙了）
        elif current_sinr < 0:
            reward -= 5
        
        # 3. 检查是否在矿道内（惩罚穿墙）
        # 如果当前位置的最强信号太弱，说明可能在墙里或者离开了矿道
        best_rssi = max([self._calculate_rssi(i) for i in range(self.num_aps)])
        if best_rssi < -85:
            reward -= 15  # 严重惩罚无法接收到好信号的位置
        elif best_rssi < -75:
            reward -= 8
        
        # 4. 检查当前AP是否与STA在同一矿道
        current_ap_pos = np.array(self.aps[self.current_ap_idx][:2])
        if not self._are_in_same_tunnel(self.position, current_ap_pos):
            reward -= 10  # 严重惩罚：连接的AP不在同一矿道（穿墙连接）
        
        # 5. AP位置奖励：如果当前AP更接近目标，给予奖励
        ...
        # 6. 奖励沿着矿道路径的AP选择
        # 如果选择的AP在通向目标的合理路径上，给予奖励
        if self._are_in_same_tunnel(current_ap_pos, self.target_position):
            reward += 3  # 奖励选择与目标在同一矿道的AP
